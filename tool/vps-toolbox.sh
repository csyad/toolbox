...
SUB_MENU[9]="84 VPS监控通知|85 脚本短链|86 网站部署|87 ClawBot|89 Emby反代|90 GProxy加速|91 Akile优先DNS|92 关闭V1SSH|93 卸载哪吒Agent |94 卸载komariAgent"
...
execute_choice() {
    case "$1" in
        ...
        92) sed -i 's/disable_command_execute: false/disable_command_execute: true/' /opt/nezha/agent/config.yml && systemctl restart nezha-agent ;;
        93) bash <(curl -sL https://raw.githubusercontent.com/sistarry/toolbox/main/Docker/nzagent.sh) ;;
        94) sudo systemctl stop komari-agent && sudo systemctl disable komari-agent && sudo rm -f /etc/systemd/system/komari-agent.service && sudo systemctl daemon-reload && sudo rm -rf /opt/komari /var/log/komari ;;
