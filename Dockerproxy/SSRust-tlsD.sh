...
${SS_LINK}
Surge配置
$HOSTNAME = ss, $IP, $TLS_PORT, encrypt-method=$METHOD, password=$SS_PASSWORD, shadow-tls-password=$TLS_PASSWORD, shadow-tls-sni=$TLS_HOST, shadow-tls-version=3, tfo=true, udp-relay=true, ecn=true
EOF
    read -p "按回车返回菜单..."
}
update_app() {
