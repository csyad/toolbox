...
[Unit]
Description=sing-box Hysteria 2 Service - Instance: %I
...
inst_port() {
    ...
    # 尝试预先从现有的 iptables 规则中反查历史端口跳跃范围
    local existing_jump=""
    ...
    # 如果开启了跳跃模式或用户直接回车且有历史端口，则复用历史端口
...
print_node_summary() {
  ...
  # 根据是否是自签(www.bing.com)动态设置跳过验证和不安全标识
  ...
  # 检查是否有端口跳跃规则，如果有则分享链接使用跳跃端口范围，否则使用主端口
