...
readonly BASE_DIR="/usr/local/etc/${TEMPLATE_NAME}"
readonly XRAY_BINARY="/usr/local/bin/${TEMPLATE_NAME}"
readonly REGISTRY_FILE="${BASE_DIR}/.instances.env"

CURRENT_INSTANCE="$(hostname -s 2>/dev/null || echo "default")"
...
download_and_extract_xray() {
    ...
    mkdir -p "/usr/local/share/${TEMPLATE_NAME}"
    cp -f "$TMP_DIR/extracted/geoip.dat" "/usr/local/share/${TEMPLATE_NAME}/" 2>/dev/null || true
    cp -f "$TMP_DIR/extracted/geosite.dat" "/usr/local/share/${TEMPLATE_NAME}/" 2>/dev/null || true
}

# ================== 配置 Systemd 模板服务 ==================
setup_systemd_service() {
    info "配置 Systemd 模板服务 [${TEMPLATE_NAME}@.service]..."
    
    cat > "/etc/systemd/system/${TEMPLATE_NAME}@.service" <<EOF
[Unit]
Description=Xray HTTP Server Service (%I)
...
ExecStart=${XRAY_BINARY} run -config ${BASE_DIR}/config_%I.json
Restart=on-failure
...
restart_xray() {
    systemctl restart "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" 2>/dev/null || true
    sleep 1
    ...
    journalctl -u "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" -n 20 --no-pager || true
    return 1
}
...
configure_xray() {
    ...
    generate_link
    systemctl enable "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" 2>/dev/null || true
    restart_xray
    show_current_config
}
...
update_xray() {
    ...
    if [ -f "$REGISTRY_FILE" ]; then
        while IFS= read -r name || [ -n "$name" ]; do
            [ -z "$name" ] && continue
            systemctl restart "${TEMPLATE_NAME}@${name}" 2>/dev/null || true
        done < "$REGISTRY_FILE"
    fi
...
uninstall_xray() {
    ...
    systemctl stop "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" 2>/dev/null || true
    systemctl disable "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" 2>/dev/null || true
    ...
    if [ ! -s "$REGISTRY_FILE" ]; then
        ...
        rm -f "$XRAY_BINARY"
        rm -rf "$BASE_DIR"
        rm -rf "/usr/local/share/${TEMPLATE_NAME}"
...
menu_switch_matrix() {
    ...
    if [ -f "$REGISTRY_FILE" ]; then
        while IFS= read -r name || [ -n "$name" ]; do
            ...
            if systemctl is-active --quiet "${TEMPLATE_NAME}@${name}" 2>/dev/null; then status_str="${GREEN}运行中${RESET}"; fi
...
install_dependencies() {
    if command -v apt &>/dev/null; then
        ...
        error "未知的包管理器，请手动安装: jq, curl, wget, unzip, openssl"
        exit 1
    fi
}
...
main() {
    ...
    while true; do
        ...
        case "$choice" in
            ...
            5) systemctl start "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" &>/dev/null || true; restart_xray; pause ;;
            6) systemctl stop "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" &>/dev/null || true; info "服务已停止"; pause ;;
            7) restart_xray; pause ;;
            8) journalctl -u "${TEMPLATE_NAME}@${CURRENT_INSTANCE}" -e --no-pager || true; pause ;;
