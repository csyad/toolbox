#!/usr/bin/env bash
#
# sing-box (Shadowsocks 2022) 多实例核心控制面板
...
export TEMPLATE_NAME="mo-ss2022-sb"
export CONFIG_DIR="/etc/mo-ss2022-sb"
export BASE_DIR="/etc/mo-ss2022-sb"
export EXECUTABLE_INSTALL_PATH="/usr/local/bin/sing-box"
export DATA_BASE_DIR="/var/lib/sing-box"
export SB_DIR_BASE="/root/proxynode/Shadowsocks"
export REGISTRY_FILE="${BASE_DIR}/.instances.env"

CURRENT_INSTANCE="$(hostname -s 2>/dev/null || echo "ss2022")"
...
generate_random_password() {
  local method="${1:-2022-blake3-aes-256-gcm}"
  case "$method" in
...
[Unit]
Description=sing-box Shadowsocks 2022 Service - Instance: %I
...
inst_method() {
  ...
  local default_method="2022-blake3-aes-256-gcm"
  local default_choice=2
  
  if [[ -f "$conf_file" ]]; then
    default_method=$(jq -r '.inbounds[0].method' "$conf_file" 2>/dev/null || echo "2022-blake3-aes-256-gcm")
  ...
  if [[ "$mode" == "edit" ]]; then
    echo -e "\n${GREEN}==== [正在修改实例参数: ${instance}] ====${RESET}"
    read -rp "$(echo -e "${GREEN}是否修改加密方式？当前为 [${default_method}] [y/N]: ${RESET}")" modify_method_choice || true
    if [[ ! "$modify_method_choice" =~ ^[Yy]$ ]]; then
      EVAL_METHOD="$default_method"
      return 0
    fi
  fi

  echo "---------------------------------------------"
  echo -e "实例 [ ${instance} ] Shadowsocks 2022 加密方式选择："
  echo -e " 1) 2022-blake3-aes-128-gcm ${YELLOW}（推荐/轻量）${RESET}"
  echo -e " 2) 2022-blake3-aes-256-gcm ${YELLOW}（高强度/默认）${RESET}"
  echo -e " 3) 2022-blake3-chacha20-poly1305 ${YELLOW}（移动端友好）${RESET}"
  echo "---------------------------------------------"
  local methodInput
  read -rp "请输入选项 [1-3] (回车默认 ${default_choice}): " methodInput
  methodInput=${methodInput:-$default_choice}

  case "$methodInput" in
    1) EVAL_METHOD="2022-blake3-aes-128-gcm" ;;
    3) EVAL_METHOD="2022-blake3-chacha20-poly1305" ;;
    *) EVAL_METHOD="2022-blake3-aes-256-gcm" ;;
  esac
}
...
print_node_summary() {
  ...
  local main_port=$(jq -r '.inbounds[0].listen_port' "$conf_file" 2>/dev/null || echo "")
  local auth_pwd=$(jq -r '.inbounds[0].password' "$conf_file" 2>/dev/null || echo "")
  local method=$(jq -r '.inbounds[0].method' "$conf_file" 2>/dev/null || echo "2022-blake3-aes-256-gcm")

  local url_ip="$vps_ip"
  [[ "$vps_ip" =~ ":" ]] && url_ip="[$vps_ip]"

  local user_info=$(echo -n "${method}:${auth_pwd}" | base64 | tr -d '\n')
  local surge_txt_file="${BASE_DIR}/link_${instance}_surge.txt"

  cat > "$surge_txt_file" <<EOF
${hostname}-${instance}-SS2022 = ss, ${url_ip}, ${main_port}, encrypt-method=${method}, password=${auth_pwd}, tfo=true, udp-relay=true, ecn=true
EOF

  echo -e "\n${GREEN}== sing-box (Shadowsocks 2022) 实例${RESET}${YELLOW} [ ${instance} ]${RESET} ${GREEN}配置详情 ==${RESET}"
  echo -e "${GREEN}外网绑定 IP  :${RESET} $vps_ip"
  echo -e "${GREEN}监听端口     :${RESET} $main_port"
  echo -e "${GREEN}加密方式     :${RESET} $method"
  echo -e "${GREEN}验证密钥     :${RESET} $auth_pwd"
  echo -e "${GREEN}配置文件路径 :${RESET} $conf_file"
  echo -e "${GREEN}Surge 文本保存:${RESET} $surge_txt_file"
  echo -e "${GREEN}--------------------------------------------${RESET}"
  echo -e "${GREEN}👉 Shadowsocks 链接 (SIP002):${RESET}"
  echo -e "${YELLOW}ss://${user_info}@$url_ip:$main_port#$hostname-ss2022-${instance}${RESET}"
  echo ""
  echo -e "${GREEN}👉 Surge 专属配置格式:${RESET}"
  echo -e "${YELLOW}${hostname}-${instance}-SS2022 = ss, ${url_ip}, ${main_port}, encrypt-method=${method}, password=${auth_pwd}, tfo=true, udp-relay=true, ecn=true${RESET}"
  echo ""
}
...
instsingbox() {
  ...
  if [[ "$mode" == "edit" ]]; then
    local old_pwd=$(jq -r '.inbounds[0].password' "$conf_file" 2>/dev/null || true)
  fi

  inst_method "$CURRENT_INSTANCE" "$mode"
  inst_port "$CURRENT_INSTANCE"

  if [[ "$mode" == "edit" ]]; then
    read -rp "设置新密钥 [当前: ${old_pwd}, 回车不修改]: " auth_pwd
    auth_pwd=${auth_pwd:-$old_pwd}
  else
    read -rp "设置 Shadowsocks 密钥 (回车自动生成匹配当前算法的高强度随机密钥): " auth_pwd
    auth_pwd=${auth_pwd:-$(generate_random_password "$EVAL_METHOD")}
  fi
...
unstsingbox() {
  ...
  rm -rf "${DATA_BASE_DIR}/${CURRENT_INSTANCE}" "/root/proxynode/Shadowsocks/${CURRENT_INSTANCE}"
...
menu() {
  ...
  while true; do
    ...
    echo -e "${GREEN}===========================================${RESET}"
    echo -e "${GREEN} ◈  sing-box  Shadowsocks 2022管理面板  ◈  ${RESET}"
    echo -e "${GREEN}===========================================${RESET}"
