...
# 部署 imgli 并初始化默认配置 (已增加自定义数据目录与自动赋权)
install_imgli() {
    check_dependencies
    
    echo -e "${CYAN}====== 自定义参数配置 ======${RESET}"
    echo -ne "${YELLOW}请输入数据存放路径 [默认: /opt/imgli/data]: ${RESET}"
    ...
    # 【新增】为数据目录赋予完整读写权限，防止 SQLite 报 out of memory (14) 错误
...
IMGLI_DATA_DIR=$custom_data_dir
EOF

    # 生成 docker-compose.yml 配置文件（使用自定义的数据挂载目录）
...
show_info() {
    ...
    if [[ -f "$ENV_FILE" ]]; then
        ...
        local custom_dir_val=$(grep "^IMGLI_DATA_DIR=" "$ENV_FILE" | cut -d'=' -f2-)
